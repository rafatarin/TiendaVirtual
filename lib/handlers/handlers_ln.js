...
var secciones = [{seccion: "ropaDeportiva.webp",titulo: "ROPA DEPORTIVA"},{seccion: "calzado.webp",titulo: "CALZADO DEPORTIVO"},{ seccion: "joyeria.jpg",titulo: "JOYERIA"},{ seccion: "juguetes.jpg",titulo: "JUGUETES"},{ seccion: "libros.webp",titulo: "LIBROS"}];
...
exports.compruebaLoginUsuario = async (req, res) => {
    ...
    if (email.length > 0) {
        if (email[0].password == req.body.contraseña) {
            req.session.user = email[0].nombre;
            let identificado = `Bienvenido: ${req.session.user}`;
            res.render("inicio", {
                identificado: identificado,
            });
...
exports.descripcion = async (req, res) => {
    let resultado = await handlers_mod.descripcion(
...
exports.actualizaEnviados = async (req, res) => {
    let enviados = await handlers_mod.actualizaEnviados(req.params.id_pedido);
    res.json(enviados);
};
