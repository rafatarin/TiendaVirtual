...
exports.creaArticulo = async (req, res) => {
    let articuloNew = await handlers_mod.creaArticulo(req.body.descripcion, req.body.stock, req.body.precio);
...
exports.guardarPedidos = async (req, res) => {
    if (req.session.user) {
        let guardarPedidos = await handlers_mod.guardarPedidos()
        res.render("pedidosGuardados", {layout: false,
            datosGuardados: guardarPedidos
        })
    } else (
        res.render("inicio")
        )
