...
exports.guardarPedidos = async () => {
  let conn;
  try {
    conn = await require("../conn.js");
    let guardarPedidos = conn.query(
      `select p.id_pedido, a.descripcion, d.cantidad, c.id_cliente, a.precio from pedidos p inner join detalle_pedidos d inner join articulos a inner join clientes c on p.id_pedido = d.id_pedido and d.id_articulo = a.id_articulo and c.id_cliente = p.id_cliente where d.pedidos_en_proceso = false and d.pedidos_enviados = false and d.pedidos_entregados = false and d.pedidos_guardados = true;`
    );
