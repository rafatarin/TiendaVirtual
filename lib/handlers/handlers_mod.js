...
exports.creaArticulo = async (descripcion, categoria, stock, precio, imagen) => {
  let conn;
  try {
    conn = await require("../conn.js");
    let resultado = await conn.query(
      `insert into articulos (descripcion, categoria, stock, precio, imagen) values ("${descripcion}", "${categoria}", ${stock}, ${precio}, "${imagen}");`
    );
    return resultado;
  } catch (err) {
    throw err;
