...
exports.mostrarPerfil = async (usuario) => {
  ...
  try {
    conn = await require("../conn.js");
    let resultado = await conn.query(`SELECT * FROM clientes WHERE usuario="${usuario}";`);
    return resultado
  } catch (err) {
    throw err;
  } finally {
...
exports.actualizarPerfil = async (usuario, identificacion) => {
  ...
  try {
    ...
    conn = await require("../conn.js");
    let resultado = await conn.query(`UPDATE clientes SET nombre="${usuario[0][0]}", apellido1="${usuario[0][1]}", apellido2="${usuario[0][2]}", usuario="${usuario[0][3]}", email="${usuario[0][4]}", poblacion="${usuario[0][5]}" WHERE usuario="${identificacion}" `);
    return resultado
  } catch (err) {
    throw err;
  } finally {
...
exports.registrarUsuario = async (array) => {
  ...
  try {
    ...
    let resultado = await conn.query(
      `INSERT INTO clientes (nombre, apellido1, apellido2, usuario, email, password, poblacion) VALUES ('${array[0][0]}','${array[0][1]}','${array[0][2]}','${array[1][0]}','${array[2][0]}','${array[2][1]}','${array[1][1]}' );`);
    return resultado;
  } catch (err) {
    throw err;
  } finally {
